 ...
 * Author : diego
 ...
 Para el ejercicio lo que se hará es hacer una tablita con las diferentes frecuencias que se puede alcanzar con una escala específica:
 - Escala de 1024: Calcular 10 frecuencias con esta escala.
 Incluir dos botones, uno que incremente y otro que decremente la frecuencia para que se observe en un led, además de mostrar en el LCD la frecuencia utilizada en el led.
 ...
 Pin 12	(PB4); Botón de - ; Para bajar frecuencia
 Pin 13	(PB5); Botón de + ; Para subir frecuencia
...
int limite_conteo = 31249;	//Valor de OCR#A inicial, para dar una frecuencia de 300.4807 [Hz]
int lista_frecuencias = 1;	//Variable que se aumenta de valor cuando se presiona un botón del Puerto B.
int botonPuertoB = 0;		//Variable que almacena lo presionado en el Puerto B.
...
int	  f = 1;				//Variable que guarda la frecuencia correspondiente a cada valor del registro OCR#A con una escala de 1024.
char  f_string[10];			//Variable guarda una cadena de caracteres representado como un array de caracteres de máximo 10 caracteres, osea un tipo de string.
...
int main(void){
	...
	//0: Pin como entrada
	//1: Pin como salida
	DDRB = (0<<DDB4);	//Pin 12 (PB4) del Puerto B como entrada, frecuencia variable;				Botón - para bajar frecuencia.
	DDRB = (0<<DDB5);	//Pin 13 (PB5) del Puerto B como entrada, frecuencia variable;				Botón + para subir frecuencia.
	...
	/*Se utilizará el comparador CTC para obtener 10 frecuencias diferentes usando la escala de 1024, con un botón se subirá la frecuencia y con otro se bajará, ya que permite 
	variar la frecuencia de la señal de salida siguiendo la fórmula descrita a continuación:
	frecuencia = fmicro / (2*escala*(1+OCR#A)) = (16X10^6) / (2*1024*(1+OCR#A))
	Por lo tanto, se logrará obtener varias frecuencias a través de la variación del registo OCR#A:
	0.- frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+OCR#A))
	1.- OCR#A = (fmicro/(frecuencia*2*escala)) - 1 = (16X10^6/(frecuencia*2*escala)) - 1
		1.-	frecuencia = 1		[Hz]
			1.- OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(1*2*256)) - 1 = 31,249 = 31249
			1.- frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+31249)) = 1 Hz
		2.-		frecuencia = 100	[Hz]
			2.-	 OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(100*2*256)) - 1 = 311.5 = 311
			2.-	 frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+OCR#A)) = 100.1602 Hz
		3.-		frecuencia = 200	[Hz]
			3.-	 OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(200*2*256)) - 1 = 155.25 = 155
			3.-	 frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+OCR#A)) = 200.3205 Hz
		4.-		frecuencia = 300	[Hz]
			4.-	 OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(300*2*256)) - 1 = 103.1666 = 103
			4.-	 frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+OCR#A)) = 300.4807 Hz
		5.-		frecuencia = 400	[Hz]
			5.-	 OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(400*2*256)) - 1 = 77.125 = 77
			5.-	 frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+OCR#A)) = 400.6410 Hz
		6.-		frecuencia = 500	[Hz]
			6.-	 OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(500*2*256)) - 1 = 61.5 = 61
			6.-	 frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+OCR#A)) = 504.0322 Hz
		7.-		frecuencia = 600	[Hz]
			7.-	 OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(600*2*256)) - 1 = 51.0833 = 51
			7.-	 frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+OCR#A)) = 600.9615 Hz
		8.-		frecuencia = 700	[Hz]
			8.-	 OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(700*2*256)) - 1 = 43.6428 = 43
			8.-	 frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+OCR#A)) = 710.2272 Hz
		9.-		frecuencia = 900	[Hz]
			9.-	 OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(800*2*256)) - 1 = 30.0625 = 31
			9.-	 frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+31)) = 976.5625 Hz
		10.-	frecuencia = 1000	[Hz] = 1 [kHz]
			10.- OCR#A = (16X10^6/(frecuencia*2*escala)) - 1 = (16X10^6/(1X10^3*2*256)) - 1 = 30.25 = 30
			10.- frecuencia = fmicro/(2*escala*(1+OCR#A)) = (16X10^6)/(2*escala*(1+30)) = 1008.0645 Hz*/
    while (1){
		//RECOPILAR LO PRESIONADO EN EL PUERTO B PARA SUBIR O BAJAR LA FRECUENCIA:
		botonPuertoB = PINB; //Lo que venga presionado del PINB lo almacena en la variable botonPuertoB
		//Se recomienda que cuando se hace una asignación al PIN, se le de un tiempo de 2 NOP mínimo, osea 2 microsegundos, para que el puerto se regule.
		...
		if (botonPuertoB == 0X20){ //Botón de + = 0010 0000 = 0X20; Lista de solo 10 frecuencias
			lista_frecuencias++; //Se aumenta el valor de la lista de frecuencias.
		}
		else if(botonPuertoB == 0X10){ //Botón de - = 0001 0000 = 0X10; Lista de solo 10 frecuencias
			lista_frecuencias--; //Se aumenta el valor de la lista de frecuencias.
		}
		else if(lista_frecuencias == 11){
			lista_frecuencias = 10; //Valor de lista_frecuencias regresado a su valor máximo permisible.
		}
		else if(lista_frecuencias == 0){
			lista_frecuencias = 1; //Valor de lista_frecuencias regresado a su valor máximo permisible.
		}
		
		//ELECCIÓN DE FRECUENCIAS PARA IMPRIMIRSE EN EL LCD Y VALORES DEL REGISTRO OCR#A
