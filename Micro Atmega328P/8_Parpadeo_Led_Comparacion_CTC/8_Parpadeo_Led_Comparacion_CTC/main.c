 ...
 * Author : diego
 8.- TEMPORIZADOR POR COMPARACIÓN: Es un tipo de temporizador que activa banderas que desencadenan interrupciones cuando el registro contador TCNT# alcanza el valor de un
 registro OCR#A (Output Compare Register T# A) u OCR#B, se le llama de comparación porque compara el valor del registro contador con alguno de los registros comparadores y
 cuando tienen el mismo valor se dispara la interrupción, existen 3 tipos de comparadores:
	- CTC (Clear to Compare), PWM rápidoy PWM fase correcta.
 Se utilizará el modo CTC (Clear to Compare), que es un modo de comparación donde cuando el temporizador alcanza el mismo nivel que el valor de comparación, se reinicia a 0 
 el valor del temporizador, para volver a realizar el conteo desde el inicio, esto modifica la señal de salida, por lo cual es posible variar su frecuencia, además es 
 importante mencionar que en el modo CTC solo se puede usar el canal A, no el B. El modo comparador CTC crea una señal de salida de diente de sierra en el micro y 
 posteriormente crea una señal cuadrada en la salida, esa es la que es de frecuencia variable.
 ...
 Para el ejercicio se configurarán 3 señales cuadradas con tres señales distintas en 3 leds diferentes con tiempos de 1s, 5s y 2.5s con los 3 timers T0, T1 y T2
 - Puerto B, Tiene 6 pines disponibles:
 Pin 8	(PB0); Led con parpadeo de 1s
 ...
 Pin A0	(PC0); Led con parpadeo de 5s
 Pin A1	(PC1)
 Pin A2	(PC2)
 Pin A3	(PC3)
 Pin A4	(PC4)
 ...
 Pin 0	(PD0); Led con parpadeo de 2.5s
 Pin 1	(PD1)
...
int main(void){
	...
	//MANEJO DEL TEMPORIZADOR: Si se quiere cambiar el inicio de conteo de un temporizador se puede modificar el estado inicial de su registro OCR#A
	...
	/*Se ejecuta el siguiente procedimiento, tomando en cuenta la frecuencia del microcontrolador, la escala, el valor máximo del timer y el tiempo que se quiere alcanzar:
		1.- conteo_OCR#A = (f_micro*t_deseado)/escala = (16X10^6*t_deseado)/escala
	Este número se redondea y es el que se debe poner en los registros OCR#A. Pero hay una condición para ello:
	Cuando: conteo_comp > conteo_total_temporizador, no se puede usar el método anterior y se debe utilizar uno completamente distinto desde cero, que de hecho igual aplica 
	para ambos casos, para ello se debe efecutar la siguiente operación, para obtener el valor de conteo unitario y ver cuantos ciclos de este conteo unitario entran en el 
	tiempo deseado que se quiere alcanzar del temporizador, el valor de los ciclos se utilizará en un condicional if dentro de la función ISR del temporizador:
		1.1.- conteo_unitario = (1/f_micro)*escala*conteo_total_temporizador
			conteo_total_temporizador T0 y T2	= 255
			conteo_total_temporizador T1		= 65,536
		1.2.- ciclos = t_deseado/conteo_unitario 
		1.3.- conteo_OCR#A = conteo_total_temporizador
	...
	Nota: 
	- En T0 y T2 el tiempo unitario mayor al que se llega con la escala más grande que es de 1024 es de 16 milisegundos, para tiempos mayores con estos temporizadores, desde 
	un inicio se deben utilizar los pasos 1.1 y 1.2
	- En T1 el tiempo unitario mayor al que se llega con la escala más grande que es de 1024 es de 4 segundos, para tiempos mayores con estos temporizadores, desde un inicio 
	se deben utilizar los pasos 1.1 y 1.2*/
	
	/*T0 - Parpadeo de 1s, hacer caso a la Nota, para tiempos mayores a 16 ms con los timers T0 y T2, desde un inicio usar los pasos 1.1 y 1.2:
		1.1.- conteo_unitario = (1/f_micro)*escala*conteo_total_temporizador = (1/16X10^6)*1024*255 = 0.01632 s
		conteo_total_temporizador T0 y T2	= 255
	...
	/*T1 - Parpadeo de 5s, hacer caso a la Nota, para tiempos mayores a 4 s con el timer T1, desde un inicio usar los pasos 1.1 y 1.:
		1.1.- conteo_unitario = (1/f_micro)*escala*conteo_total_temporizador = (1/16X10^6)*1024*65,536 = 4.1942 s
		conteo_total_temporizador T0 y T2	= 255
		conteo_total_temporizador T1		= 65,535
	...
	/*T2 - Parpadeo de 2.5s, hacer caso a la Nota, para tiempos mayores a 16 ms con los timers T0 y T2, desde un inicio usar los pasos 1.1 y 1.2:
		1.1.- conteo_unitario = (1/f_micro)*escala*conteo_total_temporizador = (1/16X10^6)*1024*255 = 0.01632 s
		conteo_total_temporizador T0 y T2	= 255
		conteo_total_temporizador T1		= 65,535
...
int cliclos_t0 = 62;				//Variable que almacena el número de ciclos que se utilizarán en la función ISR del temporizador 0.
int cliclos_transcurridos_t1 = 0;	//Variable que almacena el número de ciclos que se utilizarán en la función ISR del temporizador 1.
int cliclos_t1 = 1;					//Variable que almacena el número de ciclos que se utilizarán en la función ISR del temporizador 1.
...
int cliclos_t2 = 154;				//Variable que almacena el número de ciclos que se utilizarán en la función ISR del temporizador 2.
...
ISR (TIMER0_COMPA_vect){
	...
	comparador CTC, no se debe hacer nada, ya que siempre y cuando: 
