...
Las salidas se ejecutarán utilizando solo el canal A de dos temporizadores distintos para utilizar el modo PWM Fase Correcta con T0 y Fast PWM con T1.
 ...
 Pin 8	(PB0)
 Pin 9	(PB1); OCR1A, Señal de salida PWM del timer T1, temporizador por comparación canal A; Led con frecuencia de 1kHz; T = 1ms; Fast PWM, Modo no inverso
 ...
 Pin 6	(PD6); OCR0A, Señal de salida PWM del timer T0, temporizador por comparación canal A; Led con frecuencia de 1kHz; T = 1ms; PWM Fase correcta, Modo no inverso
 Pin 7	(PD7)
...
float Porcentaje_inicial_At0 = 3.9062f;	//Valor de OCR0A inicial, PWM Fase Correcta; frecuencia de 100 [Hz], con un porcentaje inicial de 5%. La f indica un número float
float Control_dutyCycle_At0 = 0.0f;		//Aumento o disminución del duty cycle en el temporizador 0, en función del porcentaje inicial. La f indica un número float
float Porcentaje_inicial_At1 = 30.25f;	//Valor de OCR1A inicial, PWM Rápido; frecuencia de 50 [Hz], con un porcentaje inicial de 10%. La f indica un número float
float Control_dutyCycle_At1 = 0.0f;		//Aumento o disminución del duty cycle en el temporizador 0, en función del porcentaje inicial. La f indica un número float
...
char  pt0_string[4];		//Variable guarda una cadena de caracteres representado como un array de caracteres de máximo 4 caracteres, osea un tipo de string.
char  pt1_string[4];		//Variable guarda una cadena de caracteres representado como un array de caracteres de máximo 4 caracteres, osea un tipo de string.
...
ISR(INT0_vect){
	...
	/*
	//FIJAR EL PORCENTAJE MÍNIMO DEL DUTY CYCLE DE T0, BASADO EN LA FRECUENCIA INICIAL, QUE EN ESTE CASO ES DE 5% - PWM MODO FASE CORRECTA
...
ISR(INT1_vect){
	...
	/*
	//FIJAR EL PORCENTAJE MÁXIMO DEL DUTY CYCLE DE T0, BASADO EN LA FRECUENCIA INICIAL, QUE EN ESTE CASO ES DE 5% - PWM MODO FASE CORRECTA
	if(Control_dutyCycle_At0 >= 20){
		Control_dutyCycle_At0 = 20;	//Duty Cycle de 100% para T0 que tiene un porcentaje inicial de 5%.
...
-	Con el timer T1 se pueden usar los registros OCRA# o ICR1 para definir el valor TOP, por lo cual se podrán usar ambos canales A y B para obtener la señal PWM, a menos que
    ...
    while (1){
		...
		/*T0 - Fase correcta, Modo no invertido - frecuencia = 100 Hz; T = 10 ms; duty cycle inicial = 5% = 0.05; duty cycle = variable; Escala = 1024.
			Modo sin TOP personalizado definido; Como salida se pueden utilizar los canales A (OCR#A) y B (OCR#B) con cualquiera de los temporizadores T0, T1 o T2:
			- Cálculo de frecuencia, periodo, tiempo de duty cycle y conteo OCR#, el valor del ConteoMáximo_PWM depende de la resolución del temporizador:
				1.1.0.- frecuencia = fmicro / (2*escala*OCR#)
					1.1.1.-	OCR# = fmicro / (2*escala*frecuencia) = 16X10^6 / (2*1024*100) = 78.125
				1.2.0.- Periodo = T = (2*escala*OCR#) / fmicro = 1 / frecuencia = (2*1024*78.125) / 16X10^6 = 0.01 s = 10 ms
					1.2.1.-	OCR# = (T*fmicro) / (2*escala)
				1.3.- t_dutycycle = T*Porcentaje_deseado = Porcentaje_deseado / frecuencia = 0.01*0.05 = 0.0005
				1.4.- OCR#_dutycycle = (t_dutycycle*fmicro) / (2*escala) = (0.0005*16X10^6) / (2*1024) = 3.9062
		...
		/*T1 - Fast PWM, Modo no invertido - frecuencia = 50 Hz; T = 20 ms; duty cycle inicial = 10% = 0.1; duty cycle = variable; Escala  = 1024.
			Modo sin TOP personalizado definido; Como salida se pueden utilizar los canales A (OCR#A) y B (OCR#B) con cualquiera de los temporizadores T0, T1 o T2:
			- Cálculo de frecuencia, periodo, tiempo de duty cycle y conteo OCR#, el valor del ConteoMáximo_PWM depende de la resolución del temporizador:
				1.1.0.- frecuencia = fmicro / (escala * (1 + OCR#))
					1.1.1.- OCR# = (fmicro / (frecuencia*escala)) - 1 = (16X10^6 / (50*1024)) - 1 = 311.5
				1.2.0.- Periodo = T = (escala*(1 + OCR#)) / fmicro = 1 / frecuencia = (1024*(1 + 311.5)) / 16X10^6 = 0.02 = 20 ms
					1.2.1.- OCR# = ((T*fmicro) / escala)) - 1
				1.3.- t_dutycycle = T*Porcentaje_deseado_decimal = Porcentaje_deseado_decimal / frecuencia = 0.02*0.1 = 0.002
				1.4.- OCR#_dutycycle = ((t_dutyycle*fmicro) / escala) - 1 = ((0.002*16X10^6) / 1024) - 1 = 30.25
			- Cálculo del valor que tendrá el registro de conteo del canal A o B dependiendo del modo invertido o no invertido con el que se haya configurado la señal de 
			salida PWM, OCR#I_dutycycle = Modo invertido;	OCR#NI_dutycycle = Modo no invertido:
				2.1.- OCR#I_dutycycle = ConteoMáximo_PWM - OCR#_dutycycle
