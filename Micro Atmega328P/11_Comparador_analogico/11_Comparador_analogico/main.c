...
ISR(ADC_vect){
	...
	PORTB = ADCH;			//Asingación de los 2 bits más significativos de la conversión ADC.
...
int main(void)
{
	...
	existe un solo convertidor analógico digital. El multiplexor se controla con los 4 bits MUX3, MUX2, MUX1 y MUX0:
		...
		mismo microcontrolador cuando se utiliza en condiciones de temperatura extrema, pero su desventaja es que no es muy preciso, teniendo una sensibilidad de ±10°C.
			...
			- MUX1 = 0
			- MUX0 = 0*/
	ADMUX |= (0<<MUX3)|(0<<MUX2)|(0<<MUX1)|(0<<MUX0); //ADC0: Pin A0 (PC0) como entrada analógica; MUX3 = 0, MUX2 = 0, MUX1 = 0, MUX0 = 0
	...
	ADMUX |= (1<<ADLAR); //Referencia Analógica Interna; REFS1 = 1, REFS0 = 1
	...
	del ancho de banda de la señal.
		...
		Solamente se puede usar con el timer 1 este tipo de muestreo.
			...
			- ADTS1 = 1
			- ADTS0 = 1*/
	ADCSRB |= (1<<ADTS2)|(1<<ADTS1)|(1<<ADTS0); //Modo de corrida libre; ADTS2 = 0, ADTS1 = 0, ADTS0 = 0
